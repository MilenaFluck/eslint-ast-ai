import { AfterViewInit, ChangeDetectionStrategy, Component, DoCheck, ElementRef, forwardRef, Input, OnDestroy, ViewChild } from '@angular/core';
...
export class CodemirrorEditorComponent implements AfterViewInit, OnDestroy, ControlValueAccessor, DoCheck {
  ...
  private innerExtensions = [
    EditorView.updateListener.of((viewUpdate: ViewUpdate) => this.handleValueChanged(viewUpdate)),
  ...
  writeValue(value: string): void {
    ...
    if (value !== currentValue && normalizeLineEndings(currentValue) !== normalizeLineEndings(value)) {
      this.value = value;
  ...
  setDisabledState(isDisabled: boolean) {
    ...
    this.editor.dispatch({
      effects: this.disabledCompartment.reconfigure(EditorState.readOnly.of(isDisabled)),
    });
  }
}
